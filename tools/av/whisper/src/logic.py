...
from .config import SYSTEM, SR, WIN_SEC, HOP_SEC, ENERGY_THRESH, MODEL_CACHE_DIR
...
class Pipeline:
    ...
    def run(
        ...
        from .utils import ensure_wav_mono16k, srt_timestamp, build_chunks, align_segments
        ...
        log_cb(f"進行分群... {len(embeddings)} 個片段")
        ...
        for i, (st, ed) in enumerate(chunks):
            if stop_event and stop_event.is_set():
                del model
                raise RuntimeError("使用者已取消任務")
            progress_cb(3, ((i + 1) / total_chunks) * 100, f"{i+1}/{total_chunks}")
            chunk_wav = wav[int(st * SR):int(ed * SR)].astype(np.float32)
            segs, _ = model.transcribe(chunk_wav, language=lang, beam_size=1, vad_filter=False, condition_on_previous_text=False)
            for s in segs:
